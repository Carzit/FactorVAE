...
class FeatureExtractor(nn.Module):
    ...
    def forward(self, x):# input: [seq_len, num_stocks, input_size(num_features)]
        _, h = self.gru_layer(x) # -> h: [num_layers, batch_size, hidden_size]
...
class FactorVAE(nn.Module):
    ...
    def __init__(self, 
        ...
        self.encoder = FactorEncoder(input_size=num_gru_layers*gru_hidden_size, 
                                     hidden_size=hidden_size,
                                     latent_size=latent_size)
        self.predictor = FactorPredictor(input_size=num_gru_layers*gru_hidden_size,
                                         hidden_size=hidden_size,
                                         latent_size=latent_size)
        self.decoder = FactorDecoder(input_size=num_gru_layers*gru_hidden_size,
                                     hidden_size=hidden_size,
                                     latent_size=latent_size)
